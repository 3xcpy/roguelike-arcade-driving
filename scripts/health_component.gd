...
@export var hurt_sound = null
@export var heal_sound = null
...
func damage(dmg: float) -> void:
	...
	if health <= 0:
		...
		if hurt_sound != null:
			audio_playback.play_stream(hurt_sound, 0, 0, randf_range(0.9, 1.1), AudioServer.PlaybackType.PLAYBACK_TYPE_DEFAULT, &"SFX")
	damaged.emit(dmg)
